...
def save_custom_spread(
    ...
    pos_data = [
        {"index": i, "label": label, "hint": hint}
        for i, (label, hint) in enumerate(positions)
    ]
    layout = _auto_layout(len(positions))
    ...
    try:
        conn.execute(
            ...
            "(id, name_zh, description, card_count, positions_json, layout_json, tips, created_at) "
            "VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
            (
                spread_id,
                name_zh,
